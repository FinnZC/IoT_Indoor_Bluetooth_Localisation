...
def computeResult(beaconsMap, discoveredBeacons):
    ...
    elif len(discoveredBeacons) == 2 or len(discoveredBeacons) == 1:
        try:
            # Interpolating 2 or 1 beacons with the last estimated position
    ...
    if len(readingsResponse.json()) > 0:
        ...
        for item in readingsResponse.json()[::-1]:
            try:
                ...
                timeDif =timeDifference(timeReference, getSeconds(timestamp))
                #print(timeDif)
                #print(timestamp + " | " + deviceMac + " | " + str(rssi))
                if  timeDif <= timeWindow and timeDif >= 0:
                    #
