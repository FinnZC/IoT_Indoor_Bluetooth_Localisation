...
def testA():
    beaconsTestMap = {
        ...
        "F15576CB0CF8": Beacon("F15576CB0CF8", 55.944432116316044, -3.186904862523079, -96.0+5),
    ...
    a = "F15576CB0CF8"
    rssi_a = -96
    beaconsTestMap[a].pastRssi.append(rssi_a)
    test[a] = beaconsTestMap[a]
    b = "F17FB178EA3D"
    rssi_b = -96
    beaconsTestMap[b].pastRssi.append(rssi_b)
    test[b] = beaconsTestMap[b]
    c = "FD8185988862"
    rssi_c = -100
    beaconsTestMap[c].pastRssi.append(rssi_c)
    test[c] = beaconsTestMap[c]
    for deviceMac in test:
        #discoveredBeacons[deviceMac].debug()
        #print(test[deviceMac].pastRssi)
        print(test[deviceMac].deviceMac + "," + str(test[deviceMac].getDistanceToBeacon()))
    setUpCircleRadius(test)
    lat, lng = getTrilaterationResult(test)
